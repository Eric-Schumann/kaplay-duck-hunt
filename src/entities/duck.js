...
export default (duckId, speed) => {
  ...
  const duck = k.add([
    ...
    k.area({ shape: new k.Rect(k.vec2(0), 24, 25) }),
    // Needed ? k.body(),
    k.anchor("center"),
      ...
      setBehavior() {
        ...
        this.onStateUpdate("fall", async () => {
          ...
          if (this.pos.y > k.height() - 10) {
            ...
            await k.wait(1);
            gm.enterState("duck-hunted");
          }
        });

        this.onClick(() => {
          if (gm.numBullets < 0) return;
          gm.currentScore += 100;
          this.play("shot");
